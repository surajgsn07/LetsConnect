...
const createRequest = async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, newRequest, "request made successfully"));
};
...
const updateRequestStatus = asyncHandler(async (req, res) => {
  ...
  if (isAccepted) {
    ...
    await User.findByIdAndUpdate(request.from._id, {
      $push: { connections: request.to._id },
    });
    await User.findByIdAndUpdate(request.to._id, {
      $push: { connections: request.from._id },
    });
  } else if (isRejected) {
...
const getRequests = async (req, res) => {
  ...
  try {
    const requests = await Request.find({
      ...
      isRejected: false,
      isAccepted: false,
    }).populate("from", "username profilePicture"); // Adjust the fields as necessary
...
const addEducation = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, user, "Education added successfully"));
});

// Edit education of user
const editEducation = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  const { educationId, startDate, endDate, university, qualification } =
    req.body;
...
const addExperience = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, user, "Experience added successfully"));
});
...
const editExperience = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, user, "Experience updated successfully"));
});
...
const getSentRequests = asyncHandler(async (req, res) => {
  ...
  const sentRequests = await Request.find({ from: userId }).populate(
    "to",
    "username email profileImage"
  ); // Adjust fields to populate as needed
...
const deleteConnection = asyncHandler(async (req, res) => {
  ...
  user1.connections = user1.connections.filter(
    (conn) => conn.toString() !== userId2
  );
  user2.connections = user2.connections.filter(
    (conn) => conn.toString() !== userId1
  );
